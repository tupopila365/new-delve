...
vi.mock('@delve/database', () => ({
  prisma: {
    ...
    deal: { count: vi.fn(), groupBy: vi.fn(), findMany: vi.fn(), findUnique: vi.fn(), update: vi.fn(), updateMany: vi.fn() },
    dealReport: { count: vi.fn() },
    booking: { count: vi.fn(), findMany: vi.fn(), findUnique: vi.fn() },
...
describe('admin marketplace HTTP auth', () => {
  let server: Server
  let base: string
  ...
  it('rejects traveler Bearer tokens', async () => {
    const res = await fetch(`${base}/businesses`, { headers: { Authorization: 'Bearer traveler-token' } })
    expect(res.status).toBe(401)
  })
  ...
  it('allows a valid Admin cookie session', async () => {
    ...
    const res = await fetch(`${base}/businesses?page=1&pageSize=25`, {
      headers: { cookie: `delve_admin_session=${encodeURIComponent(raw)}` },
    })
  ...
  it('rejects verification without CSRF', async () => {
    const raw = 'admin-cookie'
    vi.mocked(prisma.session.findUnique).mockResolvedValue(adminSession(raw) as never)
    vi.mocked(prisma.user.findUnique).mockResolvedValue(adminUser as never)
    const res = await fetch(`${base}/businesses/biz-a/verify`, {
      method: 'POST',
      headers: {
  ...
  it('rejects verification from an invalid origin', async () => {
    const raw = 'admin-cookie'
    vi.mocked(prisma.session.findUnique).mockResolvedValue(adminSession(raw) as never)
    vi.mocked(prisma.user.findUnique).mockResolvedValue(adminUser as never)
    const res = await fetch(`${base}/businesses/biz-a/verify`, {
