...
import { addBookingToJourney, listMyJourneys } from '../../api/journeyClient'

export default function AddBookingToJourneySheet({
  open,
  bookingId,
  bookingTitle,
  onClose,
  ...
  async function addTo(journey: JourneySummary) {
    ...
    setError(null)
    try {
      await addBookingToJourney(journey.id, bookingId)
      setDoneNote(`Added to ${journey.title}`)
      window.setTimeout(onClose, 900)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Could not add booking to journey')
    } finally {
      setBusyId(null)
    }
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {error && <p className="text-sm mb-2" style={{ color: 'var(--auth-danger)' }} role="alert">{error}</p>}
        {doneNote && <p className="text-sm mb-2" style={{ color: 'var(--primary)' }} role="status">{doneNote}</p>}
        {loading ? (
          ...
          <div className="py-10 text-center">
            ...
            <p className="text-sm m-0" style={{ color: 'var(--fg-muted)' }}>
              Create a journey first, then attach this booking.
            </p>
          </div>
        ) : (
