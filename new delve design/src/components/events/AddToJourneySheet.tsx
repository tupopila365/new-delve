...
import { addEventToJourney, listMyJourneys } from '../../api/journeyClient'

interface AddToJourneySheetProps {
  open: boolean
  eventId: string
  eventTitle: string
  onClose: () => void
  onAdded?: (journeyId: string) => void
}

export default function AddToJourneySheet({
  open,
  eventId,
  eventTitle,
  onClose,
  ...
  async function addTo(journey: JourneySummary) {
    ...
    setError(null)
    try {
      await addEventToJourney(journey.id, eventId)
      setDoneNote(`Added to ${journey.title}`)
      onAdded?.(journey.id)
      window.setTimeout(onClose, 900)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Could not add event to journey')
    } finally {
      setBusyId(null)
    }
  }
        ...
        {loading ? (
          ...
          <div className="py-10 text-center">
            ...
            <p className="text-sm m-0" style={{ color: 'var(--fg-muted)' }}>
              Create a journey first, then come back to attach this event.
            </p>
          </div>
        ) : (
              ...
              <li key={j.id}>
                <button
                  ...
                  style={{
                    ...
                    color: 'var(--fg)',
                    cursor: 'pointer',
                    opacity: busyId && busyId !== j.id ? 0.6 : 1,
                  }}
                >
