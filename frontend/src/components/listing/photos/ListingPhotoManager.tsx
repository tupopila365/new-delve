...
type Props = {
  ...
  hint?: string
  /** When true, cover slot accepts short video as well as photo. */
...
export function ListingPhotoManager({
  ...
  function setAsCover(index: number) {
    ...
    if (photoKind(item) === 'video' && !allowVideoCover) return
    const next = [...photos]
  ...
  async function onQuickPick(files: FileList | null) {
    const file = files?.[0]
    if (!file) return
    if (file.type.startsWith('video/') && photos.length === 0 && !allowVideoCover) return
    const kind = file.type.startsWith('video/') ? 'video' : 'image'
    const url = URL.createObjectURL(file)
    let posterSrc: string | undefined
    if (kind === 'video') {
      try {
        posterSrc = await videoPosterDataUrl(file, 0)
      } catch {
        posterSrc = undefined
      }
    }
    onChange([
      ...photos,
      { id: `photo-${Date.now()}`, src: url, kind, file, ...(posterSrc ? { posterSrc } : {}) },
    ])
  }
  ...
  const quickAccept =
    photos.length === 0 && !allowVideoCover
      ? 'image/*'
  ...
  return (
    <section className="listing-photos" aria-label="Photos">
        ...
        {photos.map((photo, index) => {
          ...
          return (
            <div key={photo.id} className={`listing-photos__tile${isVideo ? ' listing-photos__tile--video' : ''}`}>
              ...
              >
                {isVideo && !photo.posterSrc ? (
                  <video src={displaySrc(photo.src)} muted playsInline preload="metadata" aria-hidden />
                ) : (
              ...
              <div className="listing-photos__tile-actions">
                {index > 0 && (!isVideo || allowVideoCover) ? (
                  <button
                    type="button"
      ...
      <ListingMediaStudioSheet
        ...
        allowVideoCover={allowVideoCover}
        title={
          studio?.kind === 'edit'
