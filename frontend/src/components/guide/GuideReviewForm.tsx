...
export function GuideReviewForm({ guideId, onSubmitted }: Props) {
  ...
  const mut = useMutation({
    mutationFn: () =>
      apiFetch(`/api/guides/profiles/${guideId}/review/`, {
        method: 'POST',
        body: JSON.stringify({ rating, body: body.trim() }),
      }),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ['guide-reviews', guideId] })
      void qc.invalidateQueries({ queryKey: ['guide', String(guideId)] })
  ...
  return (
    <div className="event-review-form">
      <h3 className="event-review-form__title">How was your tour?</h3>
      <p className="event-review-form__sub">Share a quick rating to help other travellers choose a guide.</p>
