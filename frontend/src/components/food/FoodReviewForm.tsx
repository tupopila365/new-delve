...
export function FoodReviewForm({ venueId, onSubmitted }: Props) {
  ...
  const mut = useMutation({
    mutationFn: () =>
      apiFetch(`/api/food/venues/${venueId}/review/`, {
        method: 'POST',
        body: JSON.stringify({ rating, body: body.trim() }),
      }),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ['food-reviews', venueId] })
      void qc.invalidateQueries({ queryKey: ['food', String(venueId)] })
      void qc.invalidateQueries({ queryKey: ['provider-food-venues'] })
      void qc.invalidateQueries({ queryKey: ['listing-see-all', 'food', String(venueId)] })
      onSubmitted?.()
    },
  })
  ...
  return (
    <div className="event-review-form">
      <h3 className="event-review-form__title">How was your visit?</h3>
      <p className="event-review-form__sub">Share a quick rating to help other travellers choose where to eat.</p>
