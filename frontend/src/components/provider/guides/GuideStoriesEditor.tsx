...
import {
  MAX_GUIDE_STORY_CHANNELS,
  MAX_GUIDE_STORY_SLIDES,
  emptyGuideStoryChannel,
  emptyGuideStorySlide,
} from './guideStoriesFormUtils'
...
export function GuideStoriesEditor({ channels, onChange }: Props) {
  ...
  function addChannel() {
    if (channels.length >= MAX_GUIDE_STORY_CHANNELS) return
    onChange([...channels, emptyGuideStoryChannel(channels.length)])
  }

  function updateSlide(
    channelIndex: number,
    slideIndex: number,
    patch: Partial<VenueStoryChannelInput['slides'][number]>,
  ) {
  ...
  function addSlide(channelIndex: number) {
    const channel = channels[channelIndex]
    if (!channel || channel.slides.length >= MAX_GUIDE_STORY_SLIDES) return
    updateChannel(channelIndex, {
      slides: [...channel.slides, emptyGuideStorySlide(channel.slides.length)],
    })
  }
  ...
  function removeSlide(channelIndex: number, slideIndex: number) {
    ...
    updateChannel(channelIndex, { slides: slides.length ? slides : [emptyGuideStorySlide()] })
  }

  return (
    <div className="food-stories-editor">
      <p className="guide-form__hint">
        Highlight rings on your guide page — trail moments, package teasers, or behind-the-scenes. Each channel is a
        story ring travellers can tap through. Auto-generated rings still use your portfolio and packages when you
        leave this empty.
      </p>

      {channels.length === 0 ? (
        <div className="food-stories-editor__empty">
          <p>No custom story channels yet. Auto-generated rings still use your photos and packages.</p>
      ...
      {channels.map((channel, channelIndex) => (
        <div key={channel.id || `ch-${channelIndex}`} className="food-stories-editor__channel">
          <div className="food-stories-editor__channel-head">
            ...
            <button type="button" className="guide-form__icon-btn" onClick={() => removeChannel(channelIndex)}>
              Remove
            </button>
          </div>
              ...
              <div key={slide.id || `slide-${slideIndex}`} className="food-stories-editor__slide">
                ...
                <label className="guide-form__field">
                  ...
                  <input
                    ...
                    placeholder="Best light for photos"
                  />
                </label>
              </div>
            ))}
            {channel.slides.length < MAX_GUIDE_STORY_SLIDES ? (
      ...
      {channels.length > 0 && channels.length < MAX_GUIDE_STORY_CHANNELS ? (
