"""Bootstrap promotion product catalog — never overwrites admin-managed packages."""
...
# Bootstrap defaults only — Delve Admin can change prices anytime.
PLACEMENT_PRICE_CENTS = {
    ...
    PromotionPlacement.HOMEPAGE_TRANSPORT: 200_000,
...
def seed_promotion_products() -> int:
    """Create missing packages only. Existing rows (admin edits) are left alone."""
    created = 0
    duration_days = 7
    for placement in PROVIDER_PRODUCT_PLACEMENTS:
        for region in REGION_VARIANTS:
            ...
            slug = f"{placement}_{duration_days}d_{slug_region}"[:80]
            ...
            _, was_created = PromotionProduct.objects.get_or_create(
                ...
                defaults={
                    "name": name,
                    "description": _default_description(placement),
                    "placement": placement,
                    "region": region,
